...
@section('title', 'LaptopHub - Edit Supplier')
@section('active_nav', 'supplier')
@section('page_title', 'Edit Supplier')
@section('page_subtitle', 'Update supplier profile details')
...
@section('admin_content')
    ...
    <div class="form-card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.supplier.update', $supplier) }}" class="row g-3">
                @csrf
                @method('PUT')

                <div class="col-md-6">
                    <label class="form-label" for="name">Supplier Name</label>
                    <input id="name" name="name" type="text" class="form-control" value="{{ old('name', $supplier->name) }}" required>
                </div>

                <div class="col-md-6">
                    <label class="form-label" for="contact_name">Contact Person</label>
                    <input id="contact_name" name="contact_name" type="text" class="form-control" value="{{ old('contact_name', $supplier->contact_name) }}">
                </div>

                <div class="col-md-6">
                    <label class="form-label" for="contact_email">Contact Email</label>
                    <input id="contact_email" name="contact_email" type="email" class="form-control" value="{{ old('contact_email', $supplier->contact_email) }}">
                </div>

                <div class="col-md-6">
                    <label class="form-label" for="contact_phone">Contact Phone</label>
                    <input id="contact_phone" name="contact_phone" type="text" class="form-control" value="{{ old('contact_phone', $supplier->contact_phone) }}">
                </div>
                ...
                <div class="col-12">
                    ...
                    <select id="product_ids" name="product_ids[]" class="form-select" multiple size="10">
                        @foreach($products as $product)
                            <option value="{{ $product->product_id }}" @selected(collect(old('product_ids', $selectedProductIds))->contains($product->product_id))>
                                {{ $product->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-12">
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" id="is_active" name="is_active" value="1" @checked(old('is_active', $supplier->is_active))>
